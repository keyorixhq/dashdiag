...
func runBaselineSave(_ *cobra.Command, args []string) error {
	...
	_, _, snap, _ := runHealthOnce(ctx, ctrCtx, cloudEnv, profile, output.ModePlain, healthRunOpts{Terse: true}, nil)
...
func runBaselineDiff(_ *cobra.Command, args []string) error {
	...
	_, _, current, _ := runHealthOnce(ctx, ctrCtx, cloudEnv, profile, output.ModePlain, healthRunOpts{Terse: true}, nil)

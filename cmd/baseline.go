...
func runBaselineSave(_ *cobra.Command, args []string) error {
	...
	_, _, snap, _ := runHealthOnce(ctx, ctrCtx, cloudEnv, output.ModePlain, true, false, false, false, false, nil)
...
func runBaselineDiff(_ *cobra.Command, args []string) error {
	...
	_, _, current, _ := runHealthOnce(ctx, ctrCtx, cloudEnv, output.ModePlain, true, false, false, false, false, nil)

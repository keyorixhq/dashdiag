...
| 11 | openSUSE Tumbleweed | SUSE/zypper | v0.5.x; real-VMware re-run dev (`ebdd7dc`) | VM + **real VMware/vCD guest** (`.166`) | T2 | `captures/tumbleweed-data/` + local `tumbleweed-vmware-selinux-nonroot-20260629.tar.gz` | **Independently confirmed BUG-074 cross-distro (2026-06-28):** SELinux-enforcing Tumbleweed, non-root user `andrei` whose PATH lacks `/usr/sbin` (no `getenforce`) → OLD binary "kernel security module not enforced", #620 binary "SELinux enforcing". Non-root replay fixture kept local |
...
| 21 | Ubuntu 26.04 | Debian/apt | `v1.3.1-1-g1af37b7` | **real hardware (AMD Cezanne APU, 12-thread) — live USB**, host `192.168.10.36` | **T1** | live runs + dual capture 2026-06-17 (`captures/amd-ubuntu2604-20260617/`); **GPU re-run 2026-06-18** (`captures/amd-cezanne-gpu-20260618/`) | **First real AMD-silicon node.** Confirmed real: **k10temp thermal** (Tctl 45°C — first AMD thermal source in matrix), AMD **P-state** cpufreq governor (powersave, 412–4280 MHz), 12-thread load path, battery present path. **Surfaced + fixed the read-only image-fs disk false-CRIT** (iso9660 `/cdrom` at 100% → was CRIT, now OK; PR #382). **AMD GPU path COVERED (2026-06-18):** rebooted the live USB *without* `nomodeset` → amdgpu fully bound the Radeon Vega (Cezanne, `card1`, driver in use `amdgpu`). `dsd gpu` parsed real sysfs correctly — edge temp 30°C, VRAM 0.2/0.5 GB (47%, `[shared APU memory]`), power 2W, clock 400/1800 MHz, util 0%, **IsAPU=true** (GTT present → VRAM-pressure correctly suppressed); validated live + capture + offline `replay --gpu`. **Closes TRIAGE §F.** (The 2026-06-17 `nomodeset` boot had zero GPU telemetry — lesson: a `nomodeset` boot cannot test the GPU path.) NVMe SMART degrades clean w/o nvme-cli. NOT covered: NVMe SMART (no nvme-cli), persistent-disk state (live USB) |

| 22 | openSUSE Leap Micro 6.2 | SUSE/transactional-update (immutable) | dev (`ebdd7dc`) | **real VMware/vCD guest** | T2 | local `leapmicro-vmware-{20260628,postfix-20260629}.tar.gz` | **First immutable/transactional OS validated (2026-06-28).** Read-only btrfs snapshot root (`transactional-update`, `subvol=@/.snapshots/N/snapshot`). `dsd health` two-pass (root vs nobody) surfaced 3 findings, all fixed in **#620**: BUG-074 non-root SELinux false-OK (now reads `/sys/fs/selinux/enforce`), BUG-075 immutable read-only-root false-WARN, BUG-076 transactional package-install hints. Validated live; before/after capture pair kept local |
| 23 | Photon OS 5.0 | VMware/tdnf | `feat/photon-tdnf` (#558) | **real Photon 5.0** (VMware tenant `192.168.30.229`) | T2 | local | **VMware's own guest distro.** Added **tdnf CVE/security-update support (#558)** — closed a false-OK where the security/CVE scan silently no-op'd on `tdnf` (now folds to WARN; tdnf exposes no CVSS); + fixed two benign false-CRITs: per-connection `sshd@` instances and `systemd-sysupdate`. Open (backlog): Photon Tier-1 gaps (`.network` perms / `networkctl` state, iptables default-DROP correlation, boot-emergency/fsck, tdnf repo/rpmdb health) + a genuinely-failed cloud-config hidden by the ignore map |

**Family coverage:** apt/dpkg, rpm/dnf, tdnf, zypper, pacman, portage, transactional-update — all major Linux

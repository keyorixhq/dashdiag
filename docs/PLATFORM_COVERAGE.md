...
| 21 | Ubuntu 26.04 | Debian/apt | `v1.3.1-1-g1af37b7` | **real hardware (AMD Cezanne APU, 12-thread) — live USB**, host `192.168.10.36` | **T1 (partial)** | live runs + dual capture (root/non-root) 2026-06-17, `captures/amd-ubuntu2604-20260617/` | **First real AMD-silicon node.** Confirmed real: **k10temp thermal** (Tctl 45°C — first AMD thermal source in matrix), AMD **P-state** cpufreq governor (powersave, 412–4280 MHz), 12-thread load path, battery present path. **Surfaced + fixed the read-only image-fs disk false-CRIT** (iso9660 `/cdrom` at 100% → was CRIT, now OK; PR #382). Graceful-degradation paths exercised: NVMe SMART degrades clean w/o nvme-cli; GPU autodetect fires without crash on a no-telemetry device. **GPU NOT covered** — live USB booted `nomodeset`, so amdgpu loaded but never initialized the device; zero telemetry nodes (gpu_busy_percent/VRAM/hwmon all absent). Does **not** close TRIAGE §F AMD-GPU-temp item. NOT covered: GPU (see above), NVMe SMART (no nvme-cli), persistent-disk state (live USB) |

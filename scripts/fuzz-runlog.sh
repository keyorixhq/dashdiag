...
# scripts/fuzz-continuous.sh — not meant to be executed on its own.
#
# Why this exists: the rig used to write each target's output to
# /tmp/fuzz-<Target>.log, overwritten by the same target's next run and wiped
# on reboot. Between 2026-07 and 2026-08 the VCD rig raised 37 CRASH alerts and
# only one log survived long enough to be read — the rest could never be
# classified as a real bug vs. a harness flake. A log that is gone is a finding
# nobody can triage.
#
# Layout under $FUZZ_LOG_DIR (default ~/fuzz-logs):
...
#
# The box is disposable (the VCD tenant is temporary; pve01 has been
# unreachable for weeks at a time), so "kept forever on the box" is not kept
# forever. Every failing run is therefore also posted as a comment on one
# tracking issue in this repo. Posting is queued: if GitHub is unreachable or
# the token lacks the Issues permission, the entry stays in .pending-post and
# is retried after every later target, a few at a time.
#
...
FUZZ_LOG_DIR="${FUZZ_LOG_DIR:-$HOME/fuzz-logs}"
...
runlog_tracking_issue() {
  ...
  if [[ -z "$num" ]]; then
    num="$(gh issue create --title "$FUZZ_ISSUE_TITLE" --body "Opened automatically by the continuous-fuzzing rig (scripts/fuzz-continuous.sh).

Every fuzz run that exits non-zero posts its log here as a comment — whether or not it produced a \`testdata/fuzz\` reproducer. Runs that do produce one also get their own \`fuzz(crash)\` PR as before; this issue is the durable record of *every* failure, including the ones that turn out to be harness flakes, so none of them has to be triaged from a log that no longer exists.

Full logs stay on the rig under \`\$FUZZ_LOG_DIR/<Target>/\`, gzipped and never overwritten. See docs/CONTINUOUS_FUZZING.md." | grep -oE '[0-9]+$')" || return 1
  fi
  [[ -n "$num" ]] || return 1
  printf '%s\n' "$num" >"$cache"
  printf '%s\n' "$num"
...
runlog_post_one() {
  ...
  # A short hash of that line, so repeats of one failure stand out across
  # comments.

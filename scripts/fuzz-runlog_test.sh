#!/usr/bin/env bash
# Tests for scripts/fuzz-runlog.sh. Runs with fake `go` and `gh` on PATH, so
# it needs no network, no Go toolchain and no GitHub token:
#
#   bash scripts/fuzz-runlog_test.sh
#
# Runs under the same `set -euo pipefail` as fuzz-continuous.sh, because the
...
LIB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fuzz-runlog.sh"
WORK="$(mktemp -d)"
...
# (This is the case the old /tmp/fuzz-<Target>.log scheme lost.)

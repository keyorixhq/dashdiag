...
- `dsd cis --bsi`/`--nis2 --plain`/`--json` rendered a **passing** requirement
  ...
  `--yaml` emit was wrong.
- `dsd health --report`/`dsd replay --report` linked the generated markdown
  report's footer to `dashdiag.dev`, a domain the project doesn't own (the
  apex returns NXDOMAIN) — the last `dashdiag.dev` reference in the tree;
  every other renderer already pointed at `dashdiag.sh`. These reports are
  documented as "shareable" and travel to support channels and third
  parties, so the dead link went out with every report generated by v2.0.0.
...
- **zypper security-update scan false-negatived under the global zypp lock** (`dsd health`
  / `--packages` on openSUSE/SLES). dsd runs collectors in parallel, so the Packages
  collector's `zypper` could lose a race for zypper's single global lock (`/run/zypp.pid`)

...
type VaultInfo struct {
	...
	// HTTPStatus is the raw HTTP status code from /v1/sys/health.
	// 0 means the endpoint was unreachable (no response). Set even when the
	// body was present but not parseable as Vault JSON (proxy interception, wrong port).

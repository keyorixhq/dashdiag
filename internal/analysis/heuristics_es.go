...
func checkElasticsearch(e models.ElasticsearchInfo) []models.Insight {
	...
	if !e.HealthRead {
		return []models.Insight{insight("INFO", "Elasticsearch",
	...
	case "red":
		...
		return []models.Insight{insight("CRIT", "Elasticsearch", msg,
			[]string{
				"to inspect: curl -sk " + e.BaseURL + "/_cluster/allocation/explain?pretty",
				"to inspect: curl -sk " + e.BaseURL + "/_cat/shards?v | grep -v STARTED",
	...
	case "yellow":
		...
		return []models.Insight{insight("WARN", "Elasticsearch", msg,
			[]string{
				"to inspect: curl -sk " + e.BaseURL + "/_cluster/health?pretty",

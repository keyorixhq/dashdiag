...
func TestCheckMemory(t *testing.T) {
	...
	tests := []struct {
		...
		// Regression guard for internal-collectors-20-04: proc meminfo unreadable
		// (e.g. a replay bundle that never recorded it) must disclose INFO, not
		// silently score the resulting zero-valued UsedPct as a clean 0%.
		{"meminfo unreadable discloses INFO instead of scoring the zero-valued fields as clean",
			models.MemoryInfo{MeminfoUnreadable: true},
			noCtr, "INFO"},

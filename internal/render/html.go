...
type htmlReportData struct {
	...
	CVE          *htmlCVE
	Year         int
	BrandName    string       // white-label company name ("" = default dsd branding)
	BrandLogo    template.URL // self-contained logo data URI ("" = none)
}
...
<title>{{if .BrandName}}{{.BrandName}} — Health Report{{else}}DashDiag Health Report{{end}} — {{.Hostname}}</title>
<style>
  :root {
...
<div class="wrap">
  <header>
    ...
    <h1>{{if .BrandName}}Health Report{{else}}DashDiag Health Report{{end}}</h1>
    <div class="meta">

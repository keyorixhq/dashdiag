...
var parseFloatGateExemptions = map[string]string{
	...
	"internal/collectors/logs_linux.go:247":       "kmsg lookback-window boundary; a NaN/Inf timestamp makes the window filter a no-op (extra/missing log lines), not a verdict false-OK",
	"internal/collectors/logs_linux.go:276":       "kmsg entry timestamp, same shape as :243",

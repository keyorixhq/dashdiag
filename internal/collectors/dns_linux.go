...
const (
	...
	dnsNetworkManager = "NetworkManager"
)
...
func parseResolvConf(info *models.DNSResolverInfo) {
	...
	target, err := readLink(dnsResolvConf)
	if err == nil {
		info.ConfigFile = target
	} else {
		info.ConfigFile = dnsResolvConf
	}

	data, err := readFile(dnsResolvConf) // #nosec G304 -- hardcoded system file
	if err != nil {
		return
	}
...
func detectDNSManager(ctx context.Context, info *models.DNSResolverInfo) {
	...
	out, err := runCmd(ctx, "systemctl", "is-active", dnsNetworkManager)
	if err == nil && strings.TrimSpace(out) == "active" {
		// Confirm NM is actually writing resolv.conf
		data, _ := readFile(dnsResolvConf) // #nosec G304
		if strings.Contains(string(data), "Generated by NetworkManager") ||
			strings.Contains(string(data), dnsNetworkManager) {
			info.Manager = dnsNetworkManager
			return
		}
	}

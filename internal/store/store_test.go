...
func TestReadAll_TailWindowOrderedAfterManyWraps(t *testing.T) {
	...
	const total = 47
	for i := range total {
		// Verdict must stay within validateEntry's {OK,WARN,CRIT} vocabulary,
		// so per-entry identity for the ordering check below rides on Version
		// instead (unconstrained field).
		if err := s.Append(ctx, Entry{Hostname: "h", Verdict: "OK", Version: fmt.Sprintf("v%d", i)}); err != nil {
			t.Fatalf("Append: %v", err)
		}
	}
	if err := s.Close(); err != nil {
	...
	for i := range n {
		...
		if got[i].Version != want {
			t.Errorf("got[%d].Version = %q, want %q (oldest-first order of the last %d entries)", i, got[i].Version, want, n)
		}
	}
}
